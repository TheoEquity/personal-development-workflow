...
Development 进行开发？”，再由用户在当前对话明确肯定；它只把 worktree、
当前 Plan 编码和该范围内本地 commits 合并为一次授权，不扩大到远端或
finishing。合同缺失、被调用方改写或范围不足时，本入口立即停止。
...
5. **编写、评审并维护 Plan**
   ...
   - 只有 `adopt-plan` 成功后，才显示当前 `plan_ref`、配置中的实现仓库绝对路径、将由 `using-git-worktrees` 创建或确认隔离 worktree 的说明、本地 commit 范围和不含远端/finishing 的边界，然后逐字询问：**“是否开启 Subagent-Driven Development 进行开发？”** 显示后停止，不得在同一轮创建 worktree 或开始编码。
...
6. **正式 TDD 编码**
   ...
   - 用户明确回复“开启”时，该回复按 [execution-contract.md](references/execution-contract.md) 同时授权当前显示 Plan 的 worktree、编码和范围内本地 commits。立即使用 `using-git-worktrees` 检测已有隔离环境或创建一个 worktree，完成项目设置和干净基线验证；取得实际绝对 worktree 与 branch 后，把它们填入每个 Task 的同一授权合同，重新核对正式材料与实现基线，然后启动 `subagent-driven-development`。这之后不再追加一次 worktree、编码或本地 commit 确认；SDD 按其连续执行规则完成全部 Task 并返回本入口。
   ...
   - 代码实现开始前，总控必须重新核对已采用的 `spec_ref`、`plan_ref`、`test_ref`、实际 worktree `HEAD` 和适用的 AGENTS 清单是否仍与 Plan 审核输入一致；正式材料或实现基线发生漂移时返回拥有该问题的阶段，不得沿用旧批准开始编码。
...
| `tdd_coding` | 正式 Plan 刚采纳时显示 SDD 专用门禁；明确“开启”后建立或确认隔离 worktree 并连续执行 SDD，明确“不启动”或尚未选择时停在本阶段且不产生执行副作用；非 SDD 方式必须由用户后来另行明确选择 |
...
进入 `tdd_coding` 后执行主线步骤 6。若 SDD 门禁尚未回答，先显示或恢复“是否开启 Subagent-Driven Development 进行开发？”并停止。明确开启后先完成 `using-git-worktrees`，再按实际返回的 worktree 实例化合同并直接启动 SDD；明确不启动时保持停止。授权字段、作用域和执行后停止要求只由 [execution-contract.md](references/execution-contract.md) 定义；有效 RED、豁免和结构化证据只由 `test-driven-development` 定义；实现完成声明只由 `verification-before-completion` 的新鲜证据支持。
...
正式 Plan 刚采纳且 SDD 门禁尚未回答时，状态卡的阶段状态写“等待 SDD 选择”，下一步逐字写“是否开启 Subagent-Driven Development 进行开发？”。用户明确不启动后写“SDD 未启动”，不得暗示已经创建 worktree 或开始实现。
